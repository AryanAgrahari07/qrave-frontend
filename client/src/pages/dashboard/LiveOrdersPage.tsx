...
import { CheckCircle2, Clock, Utensils, ArrowRight, UserPlus, Users as UsersIcon, Check, Receipt, CreditCard, QrCode, Plus, Minus, Trash2, Loader2, RefreshCw, Printer, Share2 } from "lucide-react";
...
import { useOrders, useUpdateOrderStatus, useCancelOrder, useCreateOrder, useRestaurant, useMenuCategories, useTables, useTransactions, useAddOrderItems, useStaff } from "@/hooks/api";
...
export default function LiveOrdersPage() {
  ...
  const { data: orders, isLoading, refetch } = useOrders(restaurantId, { limit: 100 });
  ...
  const [addItemsCart, setAddItemsCart] = useState<{id: string, name: string, price: number, quantity: number}[]>([]);
  ...
  const removeFromAddItemsCart = (itemId: string) => {
    setAddItemsCart(prev => {
      const existing = prev.find(i => i.id === itemId);
      if (existing && existing.quantity > 1) {
        return prev.map(i => i.id === itemId ? { ...i, quantity: i.quantity - 1 } : i);
  ...
  const handlePayment = async (order: Order, method: string) => {
    try {
      ...
      if (order.tableId) {
        ...
        const tableOrders = (orders || []).filter((o: Order) => 
      ...
      const combinedSubtotal = ordersToBill.reduce((sum, o) => sum + parseFloat(o.subtotalAmount), 0);
      const combinedGst = ordersToBill.reduce((sum, o) => sum + parseFloat(o.gstAmount), 0);
      const combinedService = ordersToBill.reduce((sum, o) => sum + parseFloat(o.serviceTaxAmount), 0);
      const combinedTotal = ordersToBill.reduce((sum, o) => sum + parseFloat(o.totalAmount), 0);
  ...
  const handleCreateNewOrder = async () => {
    ...
    try {
      ...
      toast.success(`New order created! It will appear in the kitchen display.`);
  ...
  const handleCreateQuickBill = async (paymentMethod: string) => {
    ...
    try {
      ...
      toast.success(`Quick bill created and paid: ${currency}${billDetails.total.toFixed(2)} via ${paymentMethod}`);
  ...
  return (
    <DashboardLayout>
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6 sm:mb-8">
        <div>
          <h2 className="text-2xl sm:text-3xl font-heading font-bold">Live Orders</h2>
          <p className="text-sm sm:text-base text-muted-foreground">Monitor active orders and process payments.</p>
        </div>
        <div className="flex gap-2 w-full sm:w-auto">
          ...
          <Dialog open={isNewOrderOpen} onOpenChange={(open) => {
            ...
            <DialogTrigger asChild>
              <Button variant="outline" className="shadow-lg flex-1 sm:flex-none">
            ...
            <DialogContent className="w-[95vw] max-w-4xl max-h-[90vh] overflow-hidden flex flex-col">
              <DialogHeader>
                ...
                <DialogDescription className="text-xs sm:text-sm">Create a new order that will be sent to the kitchen for preparation.</DialogDescription>
              </DialogHeader>
              ...
              <div className="flex-1 overflow-hidden grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6 py-4">
                  ...
                  <ScrollArea className="flex-1">
                    <div className="space-y-4 sm:space-y-6">
                      {menuData?.categories?.map((category) => {
                        const items = filteredMenuItems.filter((i: MenuItem) => i.categoryId === category.id && i.isAvailable) || [];
                ...
                <div className="flex flex-col gap-4 sm:gap-6 overflow-hidden">
                  <div className="flex flex-col gap-3 flex-1 overflow-hidden">
                    <Label className="text-xs uppercase font-bold text-muted-foreground">2. Order Details</Label>
                    
                    {/* Table Selection */}
                    {orderMethod === "DINE_IN" && tables && tables.length > 0 && (
                      <div className="flex gap-2 flex-wrap">
                        {tables.filter((t: Table) => t.currentStatus === "OCCUPIED" || t.currentStatus === "AVAILABLE").slice(0, 8).map((table: Table) => (
                          <Button
                            key={table.id}
                    ...
                    {/* Waiter Selection */}
                    {staff && staff.filter((s: any) => s.role === "WAITER" && s.isActive).length > 0 && (
                      <div className="space-y-2">
                        <Label className="text-xs font-semibold">Assign to Waiter (Optional)</Label>
                        <Select value={selectedWaiterId || "none"} onValueChange={(value) => setSelectedWaiterId(value === "none" ? null : value)}>
                    ...
                    <ScrollArea className="flex-1 border rounded-lg p-2 sm:p-3 bg-muted/20">
                      <div className="space-y-2 sm:space-y-3">
                        {manualCart.length === 0 ? (
                          ...
                          manualCart.map(item => (
                            <div key={item.id} className="flex items-center justify-between group p-1.5 sm:p-2 rounded-lg hover:bg-muted/40 transition-colors">
                              <div className="flex flex-col flex-1 min-w-0">
                                <span className="text-xs sm:text-sm font-bold truncate">{item.name}</span>
                                <span className="text-[10px] sm:text-xs text-muted-foreground">{currency}{item.price} x {item.quantity} = {currency}{(item.price * item.quantity).toFixed(2)}</span>
                              </div>
                              <div className="flex items-center gap-1 sm:gap-2 shrink-0 ml-2">
                                ...
                                <span className="text-xs sm:text-sm font-bold min-w-[20px] sm:min-w-[24px] text-center bg-primary/10 px-1.5 sm:px-2 py-0.5 sm:py-1 rounded">{item.quantity}</span>
                                <Button 
                                  variant="ghost" 
                                  size="icon" 
                                  className="h-6 w-6 sm:h-7 sm:w-7 rounded-full border hover:bg-primary/10 hover:border-primary"
                                  onClick={() => addToManualCart({ id: item.id, name: item.name, price: item.price } as MenuItem)}
                                >
                    ...
                    <div className="bg-muted/30 p-2 sm:p-3 rounded-lg space-y-1.5 sm:space-y-2 text-xs sm:text-sm">
                      <div className="flex justify-between">
                        ...
                        <span>{currency}{manualCartTotal.toFixed(2)}</span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-muted-foreground">Taxes (GST {gstRate*100}% + SC {serviceRate*100}%)</span>
                        <span>{currency}{(manualCartTotal * (gstRate + serviceRate)).toFixed(2)}</span>
                      </div>
                      <div className="flex justify-between font-bold border-t pt-1.5 sm:pt-2 mt-1.5 sm:mt-2">
                        <span>Total</span>
                        <span className="text-primary text-base sm:text-lg">{currency}{(manualCartTotal * (1 + gstRate + serviceRate)).toFixed(2)}</span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="space-y-4">
                    <div className="space-y-2">
                      <Label className="text-xs uppercase font-bold text-muted-foreground">3. Select Type & Create</Label>
                      <RadioGroup 
                      ...
                      >
                        <div>
                          <RadioGroupItem value="DINE_IN" id="new-dine-in" className="peer sr-only" />
                          <Label
                            htmlFor="new-dine-in"
                        ...
                        <div>
                          <RadioGroupItem value="TAKEAWAY" id="new-takeaway" className="peer sr-only" />
                          <Label
                        ...
                        <div>
                          <RadioGroupItem value="DELIVERY" id="new-delivery" className="peer sr-only" />
                          <Label
                    ...
                    <Button 
                      ...
                      disabled={createOrder.isPending || manualCart.length === 0}
                    >
          ...
          <Dialog open={isQuickBillOpen} onOpenChange={(open) => {
            ...
            <DialogContent className="w-[95vw] max-w-4xl max-h-[90vh] overflow-hidden flex flex-col">
              <DialogHeader>
                ...
                <DialogDescription className="text-xs sm:text-sm">Add items, select method, and settle payment.</DialogDescription>
              </DialogHeader>
                  ...
                  <div className="flex flex-col gap-3 flex-1 overflow-hidden">
                    ...
                    <ScrollArea className="flex-1 border rounded-lg p-3 bg-muted/20">
                      <div className="space-y-3">
                        {manualCart.length === 0 ? (
                          ...
                          manualCart.map(item => (
                            <div key={item.id} className="flex items-center justify-between group p-2 rounded-lg hover:bg-muted/40 transition-colors">
                              <div className="flex flex-col flex-1">
                                <span className="text-sm font-bold">{item.name}</span>
                                <span className="text-xs text-muted-foreground">{currency}{item.price} x {item.quantity} = {currency}{(item.price * item.quantity).toFixed(2)}</span>
                              </div>
                              <div className="flex items-center gap-2">
                                <Button 
                                  variant="ghost" 
                                ...
                                <span className="text-sm font-bold min-w-[24px] text-center bg-primary/10 px-2 py-1 rounded">{item.quantity}</span>
                                <Button 
                                  variant="ghost" 
                                  size="icon" 
                    ...
                    <div className="bg-muted/30 p-3 rounded-lg space-y-2 text-sm">
                      ...
                      <div className="flex justify-between font-bold border-t pt-2 mt-2">
                        ...
                        <span className="text-primary text-lg">{currency}{(manualCartTotal * (1 + gstRate + serviceRate)).toFixed(2)}</span>
                      </div>
                    </div>
                  </div>
      ...
      <div className="grid lg:grid-cols-4 gap-8">
        <div className="lg:col-span-3 space-y-6">
          <h3 className="font-heading font-bold text-xl flex items-center gap-2">
            ...
            <Badge variant="outline" className="ml-2">{activeOrders.length}</Badge>
          </h3>
            ...
            <div className="space-y-4">
              {activeOrders.map((order: Order) => {
                ...
                const tableOrdersForBilling = order.tableId 
                  ? (orders || []).filter((o: Order) => o.tableId === order.tableId && o.status === "SERVED")
                  : [order];
                
                const combinedSubtotal = tableOrdersForBilling.reduce((sum, o) => sum + parseFloat(o.subtotalAmount), 0);
                const combinedGst = tableOrdersForBilling.reduce((sum, o) => sum + parseFloat(o.gstAmount), 0);
                const combinedService = tableOrdersForBilling.reduce((sum, o) => sum + parseFloat(o.serviceTaxAmount), 0);
                const combinedTotal = tableOrdersForBilling.reduce((sum, o) => sum + parseFloat(o.totalAmount), 0);
                
                return (
                  <Card key={order.id} className="overflow-hidden border-l-4 border-l-primary shadow-md hover:shadow-lg transition-shadow">
                    <CardContent className="p-6">
                      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                        <div className="space-y-2 flex-1">
                          <div className="flex items-center gap-3 flex-wrap">
                            <span className="text-xl font-bold font-heading">
                              {order.table?.tableNumber ? `Table ${order.table.tableNumber}` : order.guestName || `Order #${order.id.slice(-6)}`}
                            </span>
                            <Badge variant={getStatusColor(order.status)} className="capitalize text-xs px-2 py-1">
                          ...
                          <div className="flex items-center gap-4 flex-wrap">
                            <p className="text-sm text-muted-foreground flex items-center gap-1.5">
                              <Clock className="w-3.5 h-3.5" /> {getTimeSince(order.createdAt)}
                            </p>
                            {order.placedByStaff && (
                              <p className="text-sm text-blue-600 flex items-center gap-1.5 font-medium">
                                <UserPlus className="w-3.5 h-3.5" /> {order.placedByStaff.fullName}
                              </p>
                            )}
                          </div>
                        </div>
                        ...
                        <div className="flex-1 md:px-6">
                          <div className="flex flex-wrap gap-2">
                            {order.items?.slice(0, 4).map((item, i) => (
                              <span key={i} className="text-xs bg-muted/60 px-2.5 py-1.5 rounded-md border border-border/50 font-medium">
                                {item.quantity}x {item.itemName}
                              </span>
                            ))}
                            {(order.items?.length ?? 0) > 4 && (
                              <span className="text-xs bg-primary/10 text-primary px-2.5 py-1.5 rounded-md border border-primary/20 font-medium">
                        ...
                        <div className="flex items-center gap-4">
                          <div className="text-right mr-2">
                            <p className="text-xs text-muted-foreground uppercase font-bold tracking-tighter mb-1">Total</p>
                            <p className="text-xl font-bold font-heading text-primary">{currency}{parseFloat(order.totalAmount).toFixed(2)}</p>
                          </div>
                          ...
                          {order.status === "SERVED" && (
                            <Fragment>
                              <Dialog open={isAddItemsOpen && selectedOrder?.id === order.id} onOpenChange={(open) => {
                                ...
                                <DialogContent className="max-w-4xl max-h-[90vh] overflow-hidden flex flex-col">
                                  <DialogHeader>
                                    <DialogTitle className="flex items-center gap-2 text-2xl">
                                      ...
                                      Add More Items - {order.table?.tableNumber ? `Table ${order.table.tableNumber}` : order.guestName}
                                    </DialogTitle>
                                    <DialogDescription>Add more items to this order. They will appear in kitchen as a new order but be billed together.</DialogDescription>
                                  </DialogHeader>
                                  ...
                                  <div className="flex-1 overflow-hidden grid md:grid-cols-2 gap-6 py-4">
                                    ...
                                    <div className="flex flex-col gap-4 overflow-hidden border-r pr-6">
                                      <div className="space-y-3">
                                        <Label className="text-xs uppercase font-bold text-muted-foreground">Select Items</Label>
                                        {/* Dietary Filter */}
                                        <div className="flex gap-2">
                                          <button
                                            onClick={() => setDietaryFilter('any')}
                                                  ...
                                                  {items.map((item: MenuItem) => {
                                                    ...
                                                    return (
                                                      <Button
                                                        ...
                                                        variant="outline"
                                                        className="justify-between h-auto py-2 px-3 text-left hover:border-primary hover:bg-primary/5 group"
                                                        onClick={() => addToAddItemsCart(item)}
                                                      >
                                                        <div className="flex flex-col gap-1">
                                                          <div className="flex items-center gap-2">
                                                            <span className="font-bold text-sm">{item.name}</span>
                                    ...
                                    <div className="flex flex-col gap-6 overflow-hidden">
                                      <div className="flex flex-col gap-3 flex-1 overflow-hidden">
                                        <Label className="text-xs uppercase font-bold text-muted-foreground">Items to Add</Label>
                                                ...
                                                <div key={item.id} className="flex items-center justify-between group p-2 rounded-lg hover:bg-muted/40 transition-colors">
                                                  ...
                                                  <div className="flex items-center gap-2">
                                                    <Button 
                                                      ...
                                                      size="icon" 
                                                      className="h-7 w-7 rounded-full border hover:bg-destructive/10 hover:border-destructive"
                                                      onClick={() => removeFromAddItemsCart(item.id)}
                                                    >
                                                      <Minus className="w-3 h-3" />
                                                    </Button>
                                                    <span className="text-sm font-bold min-w-[24px] text-center bg-primary/10 px-2 py-1 rounded">{item.quantity}</span>
                                                    <Button 
                                                      ...
                                                      size="icon" 
                                                      className="h-7 w-7 rounded-full border hover:bg-primary/10 hover:border-primary"
                                                      onClick={() => addToAddItemsCart({ id: item.id, name: item.name, price: item.price } as MenuItem)}
                                                    >
                                                      <Plus className="w-3 h-3" />
                                                    </Button>
                                                  </div>
                                      ...
                                      <Button 
                                        ...
                                        disabled={addOrderItems.isPending || addItemsCart.length === 0}
                                      >
                              ...
                              <DialogContent className="max-w-5xl max-h-[90vh] overflow-hidden flex flex-col">
                                <DialogHeader>
                                  <DialogTitle className="flex items-center gap-2 text-2xl">
                                    ...
                                    Billing - {order.table?.tableNumber ? `Table ${order.table.tableNumber}` : order.guestName}
                                  </DialogTitle>
                                  <DialogDescription>
                                ...
                                <div className="flex-1 overflow-hidden grid md:grid-cols-2 gap-6 py-4">
                                  ...
                                  <div className="flex flex-col gap-4 overflow-hidden border-r pr-6">
                                    <div className="space-y-4">
                                      ...
                                      {order.table?.tableNumber && (
                                        <div className="space-y-2">
                                          <Label className="text-xs uppercase font-bold text-muted-foreground">Table</Label>
                                          <div className="bg-muted/30 p-3 rounded-lg">
                                            <div className="flex items-center gap-2">
                                              <Utensils className="w-4 h-4 text-primary" />
                                              <span className="font-bold text-lg">Table {order.table.tableNumber}</span>
                                            </div>
                                          </div>
                                        </div>
                                      )}
                                      ...
                                      {order.placedByStaff && (
                                        <div className="space-y-2">
                                          <Label className="text-xs uppercase font-bold text-muted-foreground">Waiter</Label>
                                          <div className="bg-muted/30 p-3 rounded-lg">
                                            <div className="flex items-center gap-2">
                                              <UserPlus className="w-4 h-4 text-primary" />
                                              <span className="font-semibold">{order.placedByStaff.fullName}</span>
                                            </div>
                                          </div>
                                        </div>
                                      )}
                                      ...
                                      {/* Order Type */}
                                      <div className="space-y-2">
                                        <Label className="text-xs uppercase font-bold text-muted-foreground">Order Type</Label>
                                        <div className="bg-muted/30 p-3 rounded-lg">
                                          <Badge variant="outline" className="text-sm">{order.orderType}</Badge>
                                        </div>
                                      </div>
                                      ...
                                      {tableOrdersForBilling.length > 1 && (
                                        <div className="space-y-2">
                                          <Label className="text-xs uppercase font-bold text-muted-foreground">
                                            Orders ({tableOrdersForBilling.length})
                                          </Label>
                                          <ScrollArea className="max-h-48">
                                            <div className="space-y-2">
                                              {tableOrdersForBilling.map((ord: Order, idx: number) => (
                                                <div key={ord.id} className="bg-muted/20 p-3 rounded-lg border border-border">
                                                  <div className="flex justify-between items-center mb-2">
                                                    <span className="text-xs font-bold">Order #{idx + 1}</span>
                                                    <span className="text-xs font-semibold text-primary">{currency}{parseFloat(ord.totalAmount).toFixed(2)}</span>
                                                  </div>
                                                  <div className="text-xs space-y-1">
                                                    {ord.items?.slice(0, 3).map((item, i) => (
                                                      <div key={i} className="flex justify-between">
                                                        <span className="text-muted-foreground">{item.quantity}x {item.itemName}</span>
                                                        <span>{currency}{parseFloat(item.totalPrice).toFixed(2)}</span>
                                                      </div>
                                                    ))}
                                                    {(ord.items?.length ?? 0) > 3 && (
                                                      <div className="text-muted-foreground italic pt-1">
                                                        +{(ord.items?.length ?? 0) - 3} more items
                                                      </div>
                                                    )}
                                                  </div>
                                                </div>
                                  ...
                                  <div className="flex flex-col gap-4 overflow-hidden">
                                    <div className="space-y-2">
                                      <Label className="text-xs uppercase font-bold text-muted-foreground">
                                        {tableOrdersForBilling.length > 1 ? "All Items" : "Order Items"}
                                      </Label>
                                    ...
                                    {/* Bill Summary */}
                                    <div className="space-y-2">
                                      <Label className="text-xs uppercase font-bold text-muted-foreground">Bill Summary</Label>
                                      <div className="bg-gradient-to-br from-primary/5 to-primary/10 p-4 rounded-lg space-y-2 text-sm border border-primary/20">
                                        ...
                                        <div className="flex justify-between">
                                          <span className="text-muted-foreground">GST ({gstRate*100}%)</span>
                                          <span className="font-semibold">{currency}{combinedGst.toFixed(2)}</span>
                                        </div>
                                        <div className="flex justify-between">
                                          <span className="text-muted-foreground">Service ({serviceRate*100}%)</span>
                                          <span className="font-semibold">{currency}{combinedService.toFixed(2)}</span>
                                        </div>
                                        <Separator className="my-2" />
                                        <div className="flex justify-between items-center pt-2">
                                          <span className="font-bold text-base">Total Payable</span>
                                          <span className="text-primary text-2xl font-bold">{currency}{combinedTotal.toFixed(2)}</span>
                                        </div>
                                      </div>
                                    </div>
                                  </div>
                                ...
                                <div className="text-xs text-muted-foreground italic text-center pt-2">
                                  Print and share options available after payment
                                </div>
                              </DialogContent>
                            </Dialog>
                            </Fragment>
          ...
          <div className="space-y-3">
            {transactions && transactions.length > 0 ? (
              transactions.slice(0, 5).map((transaction: any) => (
                <div key={transaction.id} className="p-3 rounded-lg border border-border bg-white shadow-sm animate-in fade-in slide-in-from-right-2">
                  <div className="flex justify-between items-start mb-1">
                    <p className="font-bold text-sm">{transaction.billNumber}</p>
                    <div className="flex gap-1">
                      {transaction.order?.orderType && (
                        <Badge className="text-[9px] bg-blue-100 text-blue-700 border-blue-200">{transaction.order.orderType}</Badge>
                      )}
                      <Badge className="text-[9px] bg-green-100 text-green-700 border-green-200">{transaction.paymentMethod}</Badge>
                    </div>
                  </div>
                  <div className="flex justify-between items-end">
                    <div>
                      <p className="text-[10px] text-muted-foreground uppercase font-bold tracking-tight">
                        {transaction.order?.table?.tableNumber ? `Table ${transaction.order.table.tableNumber}` : transaction.order?.guestName || "Guest"}
                      </p>
                      <p className="text-[10px] text-muted-foreground">{new Date(transaction.paidAt).toLocaleTimeString()}</p>
                      {transaction.order?.placedByStaff && (
                        <p className="text-[9px] text-blue-600 mt-0.5">👤 {transaction.order.placedByStaff.fullName}</p>
                      )}
                    </div>
                    <p className="font-bold text-primary font-mono text-sm">{currency}{parseFloat(transaction.grandTotal).toFixed(2)}</p>
                  </div>
                </div>
              ))
            ) : (
